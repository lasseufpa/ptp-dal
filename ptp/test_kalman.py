...
class TestKalman(unittest.TestCase):
    ...
    def run_pykalman(self, data, z, s_0, P, A, H, Q, R):
        ...
        pykf = PyKalman(transition_matrices=A,
        ...
        for i, obs in enumerate(z):
            state_mean, state_cov = pykf.filter_update(state_mean, state_cov,
                                                       obs)
    ...
    def test_kf_scalar_model(self):
        ...
        for data in self.data:
            ...
            self.assertAlmostEqual(data["y_pykf"],
                                   data["y_kf"] * 1e9,
                                   places=6)
    ...
    def test_kf_vector_model(self):
        ...
        H = np.eye(2)
        Q = np.array([[0.001, 0.005], [0.005, 0.01]])
        R = np.array([[var_x, cov_x_y], [cov_y_x, var_y]])
